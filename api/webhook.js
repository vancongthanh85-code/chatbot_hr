...
function handleHRQuery(agent) {
  ...
  if (rawPolicyName === 'trả lương' || rawPolicyName === 'tiền lương') {
    ...
    // FIX: Map tổng BHXH/BHYT/BHTN về BHTN vì nó chứa khóa 'muc_dong' chung
    policyKey = 'BHTN'; 
  ...
  if (policyKey && HR_POLICIES[policyKey]) {
    ...
    if (rawQuery.includes('mức hưởng') || rawQuery.includes('bao nhiêu') || rawQuery.includes('mức đóng') || rawQuery.includes('phần trăm')) {
      ...
      // FIX LỖI Q4: Nếu hỏi về "đóng" hoặc "phần trăm", ưu tiên trả lời Mức đóng (muc_dong)
    ...
    // 3. Xử lý Ngoại lệ Sinh Con Thứ Ba
    } else if (policyKey === 'ThaiSan' && rawQuery.includes('thứ ba')) {
    ...
    // 4. Trả lời TÓM TẮT (Nếu không có từ khóa cụ thể)
    } else {
